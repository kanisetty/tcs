...
public class Payload {
	...
	synchronized private void initializeFieldsIfNull() throws IOException {
		if(fields == null){
            if (jsonString == null) {
                AbstractOTSyncServlet.log.info("jsonString = null !!!");
            } else if (jsonString.length() == 0) {
                AbstractOTSyncServlet.log.info("jsonString = empty !!!");
            } else {
                AbstractOTSyncServlet.log.info("initializeFieldsIfNull - jsonString := " + jsonString);
            }
