...
public class ForwardingMessageListener implements SynchronousMessageListener {
	...
	public Map<String, Object> onMessage(Map<String, Object> message) throws IOException {
        ...
        AbstractOTSyncServlet.log.info("ForwardingMessageListener#onMessage() - id =" + id  +
                "  clientId=" + clientId);
		if (message.isEmpty()) {
            AbstractOTSyncServlet.log.info("ForwardingMessageListener#onMessage() - message map was empty");
        }
	...
	throws IOException {
		...
		try {
            AbstractOTSyncServlet.log.info("ForwardingMessageListener#getNewResponse() - forwarder instance of " +
                    forwarder.getClass().toString());
