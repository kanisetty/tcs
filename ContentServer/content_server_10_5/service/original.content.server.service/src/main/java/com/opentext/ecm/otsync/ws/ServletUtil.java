...
public class ServletUtil {
    ...
    public static String getHostBaseURL(HttpServletRequest request) {
        ...
        String ret = requestURL.substring(0, requestURL.length() - requestURI.length());
        return ret;
    }
    ...
    public static void write(HttpServletResponse response, String out) throws IOException {
    	...
    	PrintStream writer = new PrintStream(os, false, ServerConstants.RESPONSE_ENCODING);

    	try{
    		byte[] encodedBytes = out.getBytes(ServerConstants.RESPONSE_ENCODING);
    		response.setContentLength(encodedBytes.length);
    		response.setContentType(ServerConstants.CONTENT_TYPE);
    		response.setCharacterEncoding(ServerConstants.RESPONSE_ENCODING);
	...
	public static String getDownloadUrlForID(final String nodeID, final String vernum) {
		...
		sb.append(ServletConfig.getContentServerUrl());
		sb.append(ServletConfig.URL_FROM_NODEID_PREFIX);
		sb.append(nodeID);
		sb.append(ServletConfig.URL_FROM_NODEID_POSTFIX);
		if (vernum != null && !"".equals(vernum) && ServletUtil.isNumeric(vernum)) {
			sb.append(ServletConfig.URL_VERNUM_PREFIX);
			sb.append(vernum);
		}
