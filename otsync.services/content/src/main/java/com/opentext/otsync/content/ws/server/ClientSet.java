...
public class ClientSet {
    ...
    private List<Client> findOldClients() {
        List<Client> clientsRemoved = new ArrayList<>();
        Date now = new Date();
    ...
    public void discardOldClients() {
        ...
        clientsRemoved.forEach(this::discardAClient);

        int finalClients = size();
        if (log.isWarnEnabled() && initialClients != finalClients) {
            log.warn("Discarded some clients to free up memory: " +
                    (initialClients - finalClients) + " of " + initialClients);
        }
    }
