 ...
 * Workflow Component
 * Date: 2013-04-10
 * Dependencies: jquery, moment.js, json2.js
 *
 * action=
 * *assignment: render a workflow step
 * *view: initiate a workflow from a map
 */
...
/* global App, moment */
	$('.hide-initially').hide();
	var workflow = new App();		var appWorkCommsData = {};
	var params;		try{		if(workflow.isBB()) { 		appWorkCommsData = workflow.getJSONDataFromURL();		params = appWorkCommsData.send.data;	} else {		params = workflow.getParameters();	}		} catch(e) {		alert("param exception: " + e);	}			var object = null;	var action = params.action;	if (params.data != undefined) {			if(workflow.isBB()) { 			object = params.data;		} else {			object = JSON.parse(decodeURIComponent(params.data));		}	} else {		object = {id: params.nodeID};	}
	...
	workflow.init = function() {
		...
				if(!workflow.isBB()) { 			$('#close-button').click(workflow.close);		} else {			$('#close-button').click(workflow.returnToApp);		}
	...
	};	workflow.returnToApp = function() {				if(workflow.findComponentInArray(appWorkCommsData.onReturn.name)) {			alert(apputil.T('error.Return component not found : ' + appWorkCommsData.onReturn.name));			return;		}			var thisAppsName = appWorkCommsData.send.name;		var thisAppReturnHandler = 'onReturnFromComponent';		var destComponentName = appWorkCommsData.onReturn.name;		var destMethod = appWorkCommsData.onReturn.method;		var dataForDest = { 'data' : null};						workflow.openComponent(destComponentName, thisAppsName, destMethod, dataForDest, thisAppReturnHandler);			};	
	...
	workflow.showStep = function() {			
		if (isValidId(object.id)) {						if(workflow.isBB()) { 				try {					workflow.ajax({						url: workflow.session.baseURL + "/workflow/v1/workflows/" + object.id + "/subwork/" + object.subwork + "/tasks/" + object.step							+ "?stepDoneUrl=" + encodeURIComponent(workflow.session.baseURL + "/workflowview/stepdone.jsp?os=" + workflow.session.clientOS)							+ "&formDoneUrl=" + encodeURIComponent(workflow.session.baseURL + "/workflowview/formupdated.jsp?os=" + workflow.session.clientOS)						, success : function(data){							// If it's a signing step, stop and explain that we don't handle that.							// If it's a web form, we get a relative url back; go there.							// Otherwise, draw the page based on the data returned.							if(data.isSignatureStep){								workflow.showSignatureRequired();							}							else if(data.location != null){								window.open(workflow.session.baseURL + data.location, '_system', 'location=no');								workflow.close();							}							else {								workflow.renderStep(data);							}											}						, fail : function(error){onRequestFail(error)}					});				} catch(e)				{					workflow.setNodeName(apputil.T("label.Error") + e);				}						} else {			
				workflow.ajax({url: workflow.session.baseURL + "/workflow/v1/workflows/" + object.id + "/subwork/" + object.subwork + "/tasks/" + object.step
					+ "?stepDoneUrl=" + encodeURIComponent(workflow.session.baseURL + "/workflowview/stepdone.jsp?os=" + workflow.session.clientOS)
					+ "&formDoneUrl=" + encodeURIComponent(workflow.session.baseURL + "/workflowview/formupdated.jsp?os=" + workflow.session.clientOS)
				})
				.done(function(data) {			
	...
	workflow.showWorkflow = function(){
		if (isValidId(object.id)) {			if(workflow.isBB()) { 				try {					// BB ajax call					workflow.ajax({						url:workflow.session.baseURL + "/workflow/v1/workflows/" + object.id + "?nextUrl=" + encodeURIComponent(workflow.session.baseURL + "/workflowview/formsubmitted.jsp?os=" + workflow.session.clientOS)						, success : function(data){							// If a signature is required, stop and explain that we don't handle that.							// If it's a web form, we get a relative url back; go there.							// Otherwise, draw the page based on the data returned.							if(data.isSignatureRequired){								workflow.showSignatureRequired();							}							else if(data.location != null){								window.open(workflow.session.baseURL + data.location, '_system', 'location=no');								workflow.close();							}							else {								object.mapID = data.mapID;								workflow.renderStep(data);							}							}						, fail : function(error){onRequestFail(error)}					});				} catch(e)				{					workflow.setNodeName(apputil.T("label.Error") + e);				}						} else {				// Normal ajax call			
				workflow.ajax({url: workflow.session.baseURL + "/workflow/v1/workflows/" + object.id
					+ "?nextUrl=" + encodeURIComponent(workflow.session.baseURL + "/workflowview/formsubmitted.jsp?os=" + workflow.session.clientOS)
				})
				.done(function(data) {
					// If a signature is required, stop and explain that we don't handle that.
					// If it's a web form, we get a relative url back; go there.
	...
	workflow.renderStep = function(data){
		...
		if(data.isDueDateEditable) {
			$('#datetimepicker-due-date').datetimepicker({format: "yyyy-MM-dd HH:mm PP", pick12HourFormat: true, pickSeconds: false}).data('datetimepicker');
			$('#step-due-date').show();
		}
		
		if(data.instruction){
			$('#step-instructions').text(data.instruction).show();
		...
		if(data.comments){
			...
			if(data.comments.length == 1){
				if(data.comments[0].comment !== null && data.comments[0].comment != ''){
					$('#step-prev-comment-label').text(apputil.T("label.CommentOnPreviousStep", {step: data.comments[0].name, user: data.comments[0].user}));
					$('#step-prev-comment-text').text(data.comments[0].comment);
		...
		if(data.commentInstructions) {
			$('#commentInstructions').text(data.commentInstructions).show();
		...
		if(data.attachments){
			$('#step-view-attachments-btn').click(function(){				if(workflow.isBB()) { 									var thisAppsName = appWorkCommsData.send.name;					var thisAppReturnHandler = 'onReturnFromComponent';					var destComponentName = "objectdetailsviewbb";					var destMethod = appWorkCommsData.onReturn.method;					var dataForDest = { 'data' : null};												workflow.openComponent(destComponentName, thisAppsName, destMethod, dataForDest, thisAppReturnHandler);									} else {
					workflow.execRequest("Content", "browseObject", [data.attachments,apputil.T("label.Attachments")]);				}
			});
			if (data.attachmentInstructions != null) {
				$('#attachmentInstructions').text(data.attachmentInstructions).show();
			}
			$('#attachment-group').show();
		}
		if(data.attributes && data.attributes.length > 0){
			if(data.attributeInstructions) {
				$('#attributeInstructions').text(data.attributeInstructions).show();
			}
	...
	workflow.drawComments = function(comments){
		...
		for(var i in comments){
			...
			if(comment.comment !== null && comment.comment != ''){
				group.append($('<label id="step-prev-comment-label"/>').text(apputil.T("label.CommentOnStep", {step:comment.name , user:comment.user})))
						.append($('<p id="step-prev-comment-text">' + comment.comment + '</p>'));
			}
		}
	...
	workflow.drawForms = function(forms){
		...
		for(var i in forms){
			...
			group.append($('<button class="btn btn-block"/>').text(form.name)
					...
					.click(function(url){ return function(){
							window.open(workflow.session.baseURL + url, '_system', 'location=no');
						};}(form.url))
					);
		}
	...
	workflow.acceptAssignment = function(){
		var url = workflow.session.baseURL + "/workflow/v1/workflows/" + object.id + "/subwork/" + object.subwork + "/tasks/" + object.step;		if(workflow.isBB()) { 			try {				// BB ajax call				workflow.ajax({					url: url					, type: 'POST'					, success : function(data){						$('#accept-assignment-section').hide();						$('#step-send-btn').text('Send on').click(function(){							workflow.sendOn();						});						workflow.showStep();					}					, fail : function(error){onRequestFail(error)}				});			} catch(e)			{				workflow.setNodeName(apputil.T("label.Error") + e);			}					} else {			// Normal ajax call			workflow.ajax({					url: url,					type: 'POST'				})				.done(function(){				}).fail(onRequestFail);						}
	};
	
	workflow.initiate = function(){
		var url = workflow.session.baseURL 
			+ "/workflow/v1/workflows/" + object.mapID;
		...
		if(duedate != "") data.dueDate = moment(duedate, "YYYY-MM-DD HH:mm A").utc().format("YYYY-MM-DDTHH:mm:ss[Z]");
		...
		function onRequestFail(data) {			workflow.setNodeName(apputil.T("error.Error", {status:data.status, text:data.statusText}));			workflow.clear();					};
			...
			Date: {
				...
				if(!attr.isReadOnly){
					for (i = 0; i < attr.numRows; i++) {
						var picker;
						if (attr.isTimeField) {
							picker = $('#datetimepicker' + attr.id + '-' + i).datetimepicker({format: "yyyy-MM-dd HH:mm PP", pick12HourFormat: true, pickSeconds: false});
						}
						else {
							picker = $('#datetimepicker' + attr.id + '-' + i).datetimepicker({format: "yyyy-MM-dd", pick12HourFormat: true, pickTime: false});
						}			
				...
				for (row = 0; row < attr.numRows; row++) {
					...
					$('#attrTypeahead' + attr.id + '-' + row).typeahead({ 
						source: function(attrid, row) { 
							return function(query, process) {								if(workflow.isBB()) { 									try {										// BB ajax call										workflow.ajax({											url: workflow.session.baseURL + '/content/v4/users'											, queryParams: {filter: query}											, success : function(data){												var options = [];												for (var i in data.users) {													options.push('<span data-attrid="' + attrid + '-' + row + '" data-username="' + data.users[i].userName + '">' + workflow.format.displayName(data.users[i].userName, data.users[i].firstName, data.users[i].lastName) + '</span>');												}												return process(options);											}											, fail : function(error){onRequestFail(error)}										});									} catch(e)									{										workflow.setNodeName(apputil.T("label.Error") + e);									}											} else {									// Normal ajax call									return workflow.ajax({										url: workflow.session.baseURL + '/content/v4/users', 										data: {filter: query}									}).done( function(data) {																		var options = [];										for (var i in data.users) {											options.push('<span data-attrid="' + attrid + '-' + row + '" data-username="' + data.users[i].userName + '">' 													+ workflow.format.displayName(data.users[i].userName, data.users[i].firstName, data.users[i].lastName) + '</span>');										}										return process(options);									});																	}
		...
		workflow.baseFolders = new function() {
			...
			this.get = function() {
				if (baseFolders == null) {					if(workflow.isBB()) { 						try {							// BB ajax call							workflow.ajax({								url: workflow.session.baseURL + '/content/v4/properties'								, async: false								, type: 'GET'								, success : function(data){									var ids = [];									for (var i in data) {										ids.push(data[i]);									}									baseFolders = ids;								}								, fail : function(error){onRequestFail(error)}							});						} catch(e)						{							workflow.setNodeName(apputil.T("label.Error") + e);						}								} else {						// Normal ajax call						$.ajax({								url: workflow.session.baseURL + '/content/v4/properties', 								async: false,								cache: false,								type: 'GET'							}						).done(function(data) {							var ids = [];							for (var i in data) {								ids.push(data[i]);							}							baseFolders = ids;						});											}				
				}
				return baseFolders;
			};
