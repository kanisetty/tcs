...
;(function () {
    ...
    ObjDetails.init = function () {
        ...
        getNodeData(ObjDetails.getParameters().nodeID);
    ...
    function getNodeData (nodeId) {
        ...
        if (isValidNode(nodeId)) {
            $.when(ObjDetails.ajax({
                url: ObjDetails.session.baseURL +
                "/content/v4/nodes/" + nodeId + "/details"
            })).done(function (data) {
    ...
    function getHistory (nodeId) {
        ...
        if (isValidNode(nodeId) && $("#audit-history").children().length === 0) {
            ...
            $.when(ObjDetails.ajax({
                url: ObjDetails.session.baseURL +
                "/content/v4/nodes/" + nodeId + "/history"
            })).done(function (data) {
                if (data.ok) {
    ...
    function showTab (tabId) {
        ...
        if (tabId === "history") {
            getHistory(ObjDetails.getParameters().nodeID);
        }
    }
    ...
    function getIconUri (node) {
        ...
        return ObjDetails.session.baseURL + "/content/icons/" +
