$(document).ready(function() {
	...
	try{
		...
		if(task.isBB()) { 
			appWorkCommsData = task.getJSONDataFromURL();
			console.log(appWorkCommsData.send.data);
			console.log("appWorkCommsData.send.data : " + JSON.stringify(appWorkCommsData.send.data));
			console.log("appWorkCommsData.onReturn : " + JSON.stringify(appWorkCommsData.onReturn));
	...
	try{
		...
		alert("object exception: " + e);
	}
	
	task.init = function() {
	      
		// Make sure our assignments app is there
		task.findComponent(appWorkCommsData.onReturn.name);
		...
		if(!task.isBB()) { 
			...
			$('#close-button').click(task.returnToApp);
		}
	...
	task.returnToApp = function() {
		
		if(task.findComponentInArray(appWorkCommsData.onReturn.name)) {
		...
		task.openComponent(destComponentName, thisAppsName, destMethod, dataForDest, thisAppReturnHandler);		
	};
	...
	task.updateTask = function(){
		...
		if(errorRequiredField==''){
			...
			// TODO: Missing else ajax, perhaps a ctrl-x wrecked this?
			if(task.isBB()) { 
				try {
					task.ajax({
						type: "PUT"
						, url: task.session.baseURL + "/tasks/v1/tasks/" + object.id + str
						, success : function(){						
				...
				{
					$('#node-name').text("Error in showTask: " + e);
				}			
			} else {		
				task.ajax({
					}).done(function() {
	...
	task.showTask = function(){
		...
		if (isValidId(object.id)) {
			...
			if(task.isBB()) { 
				try {
					task.ajax({url: task.session.baseURL + "/tasks/v1/tasks/" + object.id
						, success : function(data){task.renderTaskInfo(data);}
						, fail : function(error){onRequestFail(error);}
					});
				} catch(e)
				{
				...
				task.ajax({url: task.session.baseURL + "/tasks/v1/tasks/" + object.id})
					.done(function(data){task.renderTaskInfo(data);})
					.fail(onRequestFail);
			}
		}
	};
	...
	function onRequestFail(data) {
		...
		responseText = responseText.substring((responseText.indexOf('<h1>') + 4 ),responseText.indexOf('</h1>'));
		responseText = responseText.substring((responseText.indexOf('-')));
		
		$('#node-name').text( data.status + ' ' + data.statusText + ' ' + responseText);
	};
	...
	task.renderTaskInfo = function(data){
		...
		$('#assignedTo').typeahead({
			...
			if(task.isBB()) { 
				try {
					task.ajax({
						url: task.session.baseURL + '/content/v4/users'
						, queryParams: {filter: query}
						, success : function(data){
							var options = [];
							for (var i in data.users) {
								options.push('<span data-fieldid="assignedToHidden" data-username="' + data.users[i].userName + '">' + data.users[i].userName + '</span>');
							}
