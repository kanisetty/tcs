...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="pagetitle" select="'Redefinição de senha'"/>
	<xsl:variable name="headertext" select="'Falta pouco para finalizar o processo. Precisamos apenas que você forneça uma nova senha.'"/>
	<xsl:template match="/">
		<html>
									...
									<div class="formdlg">
											...
											<div id="form-err-msg" class="error">
												<xsl:choose>
													...
													<xsl:when test="/tempoinvite/user/@token_already_used">
														<xsl:value-of select="'Você já redefiniu sua senha usando o link deste e-mail.'"/>
													</xsl:when>
										...
										<xsl:if test="not(/tempoinvite/user/@token_already_used)">
											...
											<xsl:call-template name="form-line-action">
												<xsl:with-param name="actionlabel" select="'Redefinir senha'"></xsl:with-param>
											</xsl:call-template>
										</xsl:if>
										<xsl:call-template name="form-line-spacer"></xsl:call-template>
