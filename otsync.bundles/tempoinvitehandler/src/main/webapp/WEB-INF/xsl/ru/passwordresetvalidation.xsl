...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="headertext" select="'Почти готово. Осталось указать новый пароль.'"/>
	<xsl:template match="/">
		<html>
									...
									<div class="formdlg">
											...
											<div id="form-err-msg" class="error">
												<xsl:choose>
													...
													<xsl:when test="/tempoinvite/user/@token_already_used">
														<xsl:value-of select="'Пароль уже сброшен с помощью этой ссылки в сообщении электронной почты.'"/>
													</xsl:when>
										...
										<xsl:if test="not(/tempoinvite/user/@token_already_used)">
											...
											<xsl:call-template name="form-line-action">
												<xsl:with-param name="actionlabel" select="'Сбросить пароль'"></xsl:with-param>
											</xsl:call-template>
										</xsl:if>
										<xsl:call-template name="form-line-spacer"></xsl:call-template>
