...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml">
	...
	<xsl:variable name="pagetitle" select="'Сбросить пароль'"/>
	<xsl:variable name="headertext" select="'Укажите имя пользователя. Сообщение о сбросе пароля будет отправлено на адрес электронной почты, указанный для этого пользователя.'"/>
	<xsl:template match="/">
		<html>
								...
								<form id="forgotpasswordform" method="post" action="{$base}/register/sendpasswordreset" onsubmit="return false;">
									<div class="formdlg">
											...
											<div id="form-err-msg" class="error">
												<xsl:choose>
													<xsl:when test="/tempoinvite/user/@username_missing_error">
														<xsl:value-of select="'Пользователь с таким именем не существует.'"/>
													</xsl:when>
													<xsl:when test="/tempoinvite/user/@user_error">
														<xsl:value-of select="'Заполните все обязательные поля формы.'"/>
													</xsl:when>
													<xsl:otherwise>
														<xsl:value-of select="'&#160;'"/>
													</xsl:otherwise>
											...
											<table cellpadding="0" cellmargin="0" class="formdlgfieldrow">
												<tr>
													<xsl:call-template name="form-field-input-col1">
														<xsl:with-param name="fldlabel" select="'Имя пользователя (адрес электронной почты)'"></xsl:with-param>
										...
										<xsl:call-template name="form-line-action">
											<xsl:with-param name="actionlabel" select="'Отправить'"></xsl:with-param>
										</xsl:call-template>
										<xsl:call-template name="form-line-spacer"></xsl:call-template>
