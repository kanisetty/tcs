...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="headertext" select="'Vous avez presque terminé. Il vous suffit de nous indiquer votre nom d’utilisateur et votre mot de passe.'"/>
	<xsl:template match="/">
		<html>
							...
							<div class="yui-g">
								<form id="signupexistinguser" method="post" action="{$base}/register/setupexistinguser" onsubmit="return false;">
									...
									<div class="formdlg">
											...
											<div id="form-err-msg" class="error">
												<xsl:choose>
													...
													<xsl:when test="/tempoinvite/user/@user_error">
														<xsl:value-of select="'Veuillez renseigner tous les champs.'"/>
													</xsl:when>
													<xsl:otherwise>
														<xsl:value-of select="'&#160;'"/>
													</xsl:otherwise>
										...
										<xsl:call-template name="form-line-spacer"></xsl:call-template>
										<div class="formdlgmiddle">
											<div class="formdlgalternatepath"><a href="{$base}/register/forgotpassword">Vous avez oublié votre mot de passe?</a></div>
										</div>
										<xsl:call-template name="form-line-spacer"></xsl:call-template>
										<xsl:call-template name="form-line-spacer"></xsl:call-template>
										<xsl:call-template name="form-line-action">
											<xsl:with-param name="actionlabel" select="'Configuration du compte'"></xsl:with-param>
										</xsl:call-template>
										<xsl:call-template name="form-line-spacer"></xsl:call-template>
