...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="headertext" select="'You are almost done. We just need you to provide your existing user name and password.'"/>
	<xsl:template match="/">
		<html>
							...
							<div class="yui-g">
								<form id="signupexistinguser" method="post" action="{$base}/register/setupexistinguser" onsubmit="return false;">
									...
									<div class="formdlg">
											...
											<div id="form-err-msg" class="error">
												<xsl:choose>
													...
													<xsl:when test="/tempoinvite/user/@user_error">
														<xsl:value-of select="'Please fill in all fields.'"/>
													</xsl:when>
													<xsl:otherwise>
														<xsl:value-of select="'&#160;'"/>
													</xsl:otherwise>
										...
										<xsl:call-template name="form-line-spacer"></xsl:call-template>
										<div class="formdlgmiddle">
											<div class="formdlgalternatepath"><a href="{$base}/register/forgotpassword">Forgot your password?</a></div>
										</div>
										<xsl:call-template name="form-line-spacer"></xsl:call-template>
										<xsl:call-template name="form-line-spacer"></xsl:call-template>
										<xsl:call-template name="form-line-action">
											<xsl:with-param name="actionlabel" select="'Setup Account'"></xsl:with-param>
										</xsl:call-template>
										<xsl:call-template name="form-line-spacer"></xsl:call-template>
