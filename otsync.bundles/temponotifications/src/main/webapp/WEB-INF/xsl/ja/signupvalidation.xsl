...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="pagetitle" select="'Tempo への招待アカウントの設定'"/>
	<xsl:variable name="headertext" select="'手順はほぼ終了しました。 最後にユーザー名とパスワードを設定してください。'"/>
	<xsl:template match="/">
		<html>
											...
											<div id="form-err-msg" class="error">
												<xsl:choose>
													...
													<xsl:when test="/tempoinvite/user/@token_already_used">
														<xsl:value-of select="'この E メールのリンクを使用して、お客様は既にサインアップされています。'"/>
													</xsl:when>
													...
													<xsl:when test="/tempoinvite/user/@username_exists_error">
														<xsl:value-of select="'指定したユーザー名は既に使用されています。 別のユーザー名を指定してください。'"/>
													</xsl:when>
										...
										<xsl:if test="not(/tempoinvite/user/@token_already_used)">
												...
												<table cellpadding="0" cellmargin="0" class="formdlgfieldrow">
													<tr>
														<xsl:call-template name="form-field-input-col1">
															<xsl:with-param name="fldlabel" select="'ユーザー名'"></xsl:with-param>
											...
											<div class="formdlgmiddle">
												<table cellpadding="0" cellmargin="0" class="formdlgfieldrow">
													<tr>
														<xsl:call-template name="form-field-input-col1">
															<xsl:with-param name="fldlabel" select="'名'"></xsl:with-param>
															<xsl:with-param name="fldid" select="''"></xsl:with-param>
														...
														<xsl:call-template name="form-field-input-col2">
															<xsl:with-param name="fldlabel" select="'姓'"></xsl:with-param>
															<xsl:with-param name="fldid" select="''"></xsl:with-param>
											...
											<xsl:call-template name="form-line-action">
												<xsl:with-param name="actionlabel" select="'アカウントの作成'"></xsl:with-param>
											</xsl:call-template>
										</xsl:if>
										<xsl:call-template name="form-line-spacer"></xsl:call-template>
