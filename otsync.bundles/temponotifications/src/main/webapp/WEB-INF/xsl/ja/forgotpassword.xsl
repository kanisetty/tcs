...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml">
	...
	<xsl:variable name="headertext" select="'ユーザー名が必要です。 パスワードのリセット メッセージがご使用の E メール アドレスに送信されます。'"/>
	<xsl:template match="/">
		<html>
								...
								<form id="forgotpasswordform" method="post" action="{$base}/register/sendpasswordreset" onsubmit="return false;">
									<div class="formdlg">
											...
											<div id="form-err-msg" class="error">
												<xsl:choose>
													<xsl:when test="/tempoinvite/user/@username_missing_error">
														<xsl:value-of select="'指定したユーザー名は存在しません。'"/>
													</xsl:when>
													<xsl:when test="/tempoinvite/user/@user_error">
														<xsl:value-of select="'すべての必須フォーム フィールドに入力してください。'"/>
													</xsl:when>
													<xsl:otherwise>
														<xsl:value-of select="'&#160;'"/>
													</xsl:otherwise>
										...
										<xsl:call-template name="form-line-action">
											<xsl:with-param name="actionlabel" select="'送信'"></xsl:with-param>
										</xsl:call-template>
										<xsl:call-template name="form-line-spacer"></xsl:call-template>
