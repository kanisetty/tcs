...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="head">
		<xsl:param name="title" select="'Tempo-Einladung'"/>
		<head>
	...
	<xsl:template name="branding-hd">
		...
		<div class="defaultheader">
			<img src="{$media}/ot/tempo_wordmark.png" alt="Markenüberschrift"></img>
		</div>
	</xsl:template>	
	<xsl:template name="body-hd">
		<xsl:param name="hdrtext" select="'Tempo-Einladung'"/>
	...
	<xsl:template name="body-ft">
		<div id="ft">
			<div class="footertext">
				<p>Copyright &#169; 2015 OpenText Corporation. Alle Rechte vorbehalten.</p>
			</div>
		</div>
	</xsl:template>	
	<xsl:template name="policy">
		<a target="_blank" href="http://www.opentext.com/2/global/site-privacy.html">OpenText-Datenschutzgrundsatz</a>
	</xsl:template>
						...
						<xsl:choose>
							<xsl:when test="$step = 'one'">
								<div class="formdlgstep">
									<img src="{$media}/ot/step_01.png" alt="Schritt 1"></img>
								</div>
							</xsl:when>
							<xsl:when test="$step = 'two'">
								<div class="formdlgstep">
									<img src="{$media}/ot/step_02.png" alt="Schritt 2"></img>
								</div>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="'&#160;'"/>
	...
	<xsl:template name="form-field-input-col1">
		<xsl:param name="fldlabel" select="'Feldbezeichnung'"/>
	...
	<xsl:template name="form-line-action">
		<xsl:param name="actionlabel" select="'Aktionsbezeichnung'"/>
	...
	<xsl:template name="action-button">
		<xsl:param name="btnlabel" select="'Schaltflächenbezeichnung'"/>
	...
	<xsl:template name="action-button-spinner">
		<div id="action-btn-spinner" class="actionbtnspinner" onselectstart="return false;">
			<img src="{$media}/ot/loader.gif" alt="Ausgelastet-Symbol"></img>
		</div>
	</xsl:template>
