...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="pagetitle" select="'Tempo-Einladung - Kontoeinrichtung'"/>
	<xsl:variable name="headertext" select="'Sie haben es fast geschafft. Sie müssen nur noch Ihren Benutzernamen und Ihr Kennwort einrichten.'"/>
	<xsl:template match="/">
		<html>
											...
											<div id="form-err-msg" class="error">
												<xsl:choose>
													...
													<xsl:when test="/tempoinvite/user/@token_already_used">
														<xsl:value-of select="'Sie haben sich bereits über diese E-Mail-Verknüpfung registriert.'"/>
													</xsl:when>
													...
													<xsl:when test="/tempoinvite/user/@username_exists_error">
														<xsl:value-of select="'Leider wird dieser Benutzername bereits verwendet. Wählen Sie einen anderen Namen.'"/>
													</xsl:when>
										...
										<xsl:if test="not(/tempoinvite/user/@token_already_used)">
												...
												<table cellpadding="0" cellmargin="0" class="formdlgfieldrow">
													<tr>
														<xsl:call-template name="form-field-input-col1">
															<xsl:with-param name="fldlabel" select="'Benutzername'"></xsl:with-param>
											...
											<div class="formdlgmiddle">
												<table cellpadding="0" cellmargin="0" class="formdlgfieldrow">
													<tr>
														<xsl:call-template name="form-field-input-col1">
															<xsl:with-param name="fldlabel" select="'Vorname'"></xsl:with-param>
															<xsl:with-param name="fldid" select="''"></xsl:with-param>
														...
														<xsl:call-template name="form-field-input-col2">
															<xsl:with-param name="fldlabel" select="'Nachname'"></xsl:with-param>
															<xsl:with-param name="fldid" select="''"></xsl:with-param>
											...
											<xsl:call-template name="form-line-action">
												<xsl:with-param name="actionlabel" select="'Konto erstellen'"></xsl:with-param>
											</xsl:call-template>
										</xsl:if>
										<xsl:call-template name="form-line-spacer"></xsl:call-template>
