...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="head">
		<xsl:param name="title" select="'Tempo 邀请'"/>
		<head>
	...
	<xsl:template name="branding-hd">
		...
		<div class="defaultheader">
			<img src="{$media}/ot/tempo_wordmark.png" alt="品牌标题"></img>
		</div>
	</xsl:template>	
	<xsl:template name="body-hd">
		<xsl:param name="hdrtext" select="'Tempo 邀请'"/>
	...
	<xsl:template name="body-ft">
		<div id="ft">
			<div class="footertext">
				<p>版权所有 &#169; 2015 OpenText Corporation。 保留所有权利。</p>
			</div>
		</div>
	</xsl:template>	
	<xsl:template name="policy">
		<a target="_blank" href="http://www.opentext.com/2/global/site-privacy.html">OpenText 隐私政策</a>
	</xsl:template>
						...
						<xsl:choose>
							<xsl:when test="$step = 'one'">
								<div class="formdlgstep">
									<img src="{$media}/ot/step_01.png" alt="第一步"></img>
								</div>
							</xsl:when>
							<xsl:when test="$step = 'two'">
								<div class="formdlgstep">
									<img src="{$media}/ot/step_02.png" alt="第二步"></img>
								</div>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="'&#160;'"/>
	...
	<xsl:template name="form-field-input-col1">
		<xsl:param name="fldlabel" select="'字段标签'"/>
	...
	<xsl:template name="form-line-action">
		<xsl:param name="actionlabel" select="'操作标签'"/>
	...
	<xsl:template name="action-button">
		<xsl:param name="btnlabel" select="'按钮标签'"/>
	...
	<xsl:template name="action-button-spinner">
		<div id="action-btn-spinner" class="actionbtnspinner" onselectstart="return false;">
			<img src="{$media}/ot/loader.gif" alt="“繁忙”图标"></img>
		</div>
	</xsl:template>
