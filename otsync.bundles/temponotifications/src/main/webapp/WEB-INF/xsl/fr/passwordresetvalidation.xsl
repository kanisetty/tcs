...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="headertext" select="'Vous avez presque terminé. Il vous suffit de nous fournir un nouveau mot de passe.'"/>
	<xsl:template match="/">
		<html>
									...
									<div class="formdlg">
											...
											<div id="form-err-msg" class="error">
												<xsl:choose>
													...
													<xsl:when test="/tempoinvite/user/@token_already_used">
														<xsl:value-of select="'Vous avez déjà réinitialisé votre mot de passe à l’aide de ce lien.'"/>
													</xsl:when>
										...
										<xsl:if test="not(/tempoinvite/user/@token_already_used)">
											...
											<xsl:call-template name="form-line-action">
												<xsl:with-param name="actionlabel" select="'Réinitialisation du mot de passe'"></xsl:with-param>
											</xsl:call-template>
										</xsl:if>
										<xsl:call-template name="form-line-spacer"></xsl:call-template>
