...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="pagetitle" select="'Configuración de la cuenta de invitación a Tempo'"/>
	<xsl:variable name="headertext" select="'Ya casi ha finalizado. Sólo tiene que configurar su nombre de usuario y contraseña.'"/>
	<xsl:template match="/">
		<html>
											...
											<div id="form-err-msg" class="error">
												<xsl:choose>
													...
													<xsl:when test="/tempoinvite/user/@token_already_used">
														<xsl:value-of select="'Ya se ha inscrito a través de este enlace de correo electrónico.'"/>
													</xsl:when>
													...
													<xsl:when test="/tempoinvite/user/@username_exists_error">
														<xsl:value-of select="'Lo sentimos, ese nombre de usuario ya está en uso. Seleccione otro.'"/>
													</xsl:when>
										...
										<xsl:if test="not(/tempoinvite/user/@token_already_used)">
												...
												<table cellpadding="0" cellmargin="0" class="formdlgfieldrow">
													<tr>
														<xsl:call-template name="form-field-input-col1">
															<xsl:with-param name="fldlabel" select="'Nombre de usuario'"></xsl:with-param>
											...
											<div class="formdlgmiddle">
												<table cellpadding="0" cellmargin="0" class="formdlgfieldrow">
													<tr>
														<xsl:call-template name="form-field-input-col1">
															<xsl:with-param name="fldlabel" select="'Nombre'"></xsl:with-param>
															<xsl:with-param name="fldid" select="''"></xsl:with-param>
														...
														<xsl:call-template name="form-field-input-col2">
															<xsl:with-param name="fldlabel" select="'Apellido'"></xsl:with-param>
															<xsl:with-param name="fldid" select="''"></xsl:with-param>
											...
											<xsl:call-template name="form-line-action">
												<xsl:with-param name="actionlabel" select="'Crear cuenta'"></xsl:with-param>
											</xsl:call-template>
										</xsl:if>
										<xsl:call-template name="form-line-spacer"></xsl:call-template>
