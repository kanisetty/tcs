...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="pagetitle" select="'Password Reset'"/>
	<xsl:variable name="headertext" select="'You are almost done. We just need you to provide a new password.'"/>
	<xsl:template match="/">
		<html>
									...
									<div class="formdlg">
											...
											<div id="form-err-msg" class="error">
												<xsl:choose>
													...
													<xsl:when test="/tempoinvite/user/@token_already_used">
														<xsl:value-of select="'You have already reset your password using this e-mail link.'"/>
													</xsl:when>
										...
										<xsl:if test="not(/tempoinvite/user/@token_already_used)">
											...
											<xsl:call-template name="form-line-action">
												<xsl:with-param name="actionlabel" select="'Reset Password'"></xsl:with-param>
											</xsl:call-template>
										</xsl:if>
										<xsl:call-template name="form-line-spacer"></xsl:call-template>
