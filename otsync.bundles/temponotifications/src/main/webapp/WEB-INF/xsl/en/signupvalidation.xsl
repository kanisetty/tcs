...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="headertext" select="'You are almost done. We just need you to setup your user name and password.'"/>
	<xsl:template match="/">
		<html>
								...
								<form id="signupvalidation" method="post" action="{$base}/register/create" onsubmit="return false;">
									...
									<div class="formdlg">
											...
											<div id="form-err-msg" class="error">
												<xsl:choose>
													...
													<xsl:when test="/tempoinvite/user/@token_already_used">
														<xsl:value-of select="'You have already signed up using this e-mail link.'"/>
													</xsl:when>
													<xsl:when test="/tempoinvite/user/@password_confirm_error">
														<xsl:value-of select="'Please ensure the passwords match.'"/>
													</xsl:when>
													<xsl:when test="/tempoinvite/user/@username_exists_error">
														<xsl:value-of select="'Sorry, that user name is already in use. Please choose another one.'"/>
													</xsl:when>
													<xsl:when test="/tempoinvite/user/@user_error">
														<xsl:value-of select="'Please fill in all fields. Password must be at least 6 characters long.'"/>
													</xsl:when>
													<xsl:otherwise>
														<xsl:value-of select="'&#160;'"/>
													</xsl:otherwise>
										...
										<xsl:if test="not(/tempoinvite/user/@token_already_used)">
											<div class="formdlgmiddle">
												<div class="formdlginstruction">Password must be at least 6 characters long and is case sensitive. Password must contain at least 1 number.</div>
											</div>
											<xsl:call-template name="form-line-spacer"></xsl:call-template>
											<div class="formdlgmiddle">
												<table cellpadding="0" cellmargin="0" class="formdlgfieldrow">
													<tr>
														<xsl:call-template name="form-field-input-col1">
															<xsl:with-param name="fldlabel" select="'User Name'"></xsl:with-param>
												...
												<table cellpadding="0" cellmargin="0" class="formdlgfieldrow">
													<tr>
														...
														<xsl:call-template name="form-field-input-col2">
															<xsl:with-param name="fldlabel" select="'Confirm Password'"></xsl:with-param>
															<xsl:with-param name="fldid" select="''"></xsl:with-param>
											...
											<div class="formdlgmiddle">
												<table cellpadding="0" cellmargin="0" class="formdlgfieldrow">
													<tr>
														<xsl:call-template name="form-field-input-col1">
															<xsl:with-param name="fldlabel" select="'First Name'"></xsl:with-param>
															<xsl:with-param name="fldid" select="''"></xsl:with-param>
														...
														<xsl:call-template name="form-field-input-col2">
															<xsl:with-param name="fldlabel" select="'Last Name'"></xsl:with-param>
															<xsl:with-param name="fldid" select="''"></xsl:with-param>
											...
											<xsl:call-template name="form-line-action">
												<xsl:with-param name="actionlabel" select="'Create Account'"></xsl:with-param>
											</xsl:call-template>
										</xsl:if>
										<xsl:call-template name="form-line-spacer"></xsl:call-template>
