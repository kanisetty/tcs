...
(function($) {
	...
	// fixed for OTAG/Tempo Box: pluralize when count is 0
	function needsPlural(options){
		return ("count" in options && typeof options.count != 'string' && options.count != 1);
	}
	...
	*/
	function _translate(dottedkey,options){
		...
		if(value){
			//
			// htmlDecode added for OTAG/Tempo Box; htmlDecode() is defined in app.js
			//
			value = apputil.htmlDecode(value);
	...
	function loadDictionary(lang,doneCallback){
		...
		$.ajax({
			url: [o.dicoPath,"/", lang, '.', o.dicoSuffix].join(''),
